...
function uiButton(options) {
  const preset = options.hash.presetStyle ? camelcase(options.hash.presetStyle) : 'lightBlue';
  const validOptions = {};

  Object.keys(options.hash).forEach(itm => {
    if (validValue(options.hash[itm])) {
      validOptions[itm] = options.hash[itm];
    }
  });
