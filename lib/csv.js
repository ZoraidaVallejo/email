...
class Csv {
  ...
  count(columns = []) {
    this.config = {};
    this.element.forEach(function eachOriginalValue(originalValue) {
      ...
      if (!this.config[mainKey]) {
        ...
        if (columns.length > 0) {
          columns.forEach(function eachColumn(col, idx) {
  ...
  getUtmVars(columnName) {
    this.configKeys.forEach(function eachKey(key) {
      var url = urlParse(this.config[key].table[columnName], true);

      // Rewrite original.
      this.config[key].table[columnName] = `${url.href.split('?')[0]}`;

      utmVarsOrder.forEach(function eachUtm(utm) {
        var utmKey = `utm_${utm}`;
   ...
   */
  getTable() {
    return this.configKeys.map(function getValuesOfKey(key) {
      return Object.values(this.config[key].table);
    });
  }
